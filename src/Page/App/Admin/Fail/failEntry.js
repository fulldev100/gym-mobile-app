...
import { fetchAdminDashboardEntrylist, loadingStart } from "../../../redux/actions/adminDashboard";
...
class FailEntry extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ImageLoading: false,
    ...
    async myAdmindata() {
        const { fetchAdminDashboardEntrylist, loadingStart } = this.props;
        loadingStart();
        ...
        fetchAdminDashboardEntrylist(dashboard_data);
    }
                ...
                <TouchableOpacity key={item.invoice_id} style={styleCss.TouchScreenCSS}>
                    ...
                    { 
                        item.total_entry_in_list.map((prop, key) => {
                            if (prop.card_num != "0" && prop.carryout_status == 'Verify fail')
                            return (
                    ...
                    <View style={styleCss.bottomView}>
                        ...
                        <View style={styleCss.bottomViewColumn}>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate('failEntry')} style={styleCss.message_col}>
                                <Image style={styleCss.bottomViewColumnImg}
                                    source={require('../../../../images/icons8-door-sensor-error-active.png')}
                                />
                                <Text style={styleCss.bottomViewColumnTextActive}>{t("Fail")}</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={styleCss.bottomViewColumn}>
...
const mapDispatchToProps = {
    fetchAdminDashboardEntrylist,
    Logoutmember,
    loadingStart,
};

export default connect(mapStateToProps, mapDispatchToProps)(FailEntry);